...
<div class="admin-container">
    ...
    <div class="admin-header">
        <h1 class="admin-title">Novo Ad Unit</h1>
    </div>

    <form action="{{ route('admin.ad-units.store') }}" method="POST" class="ad-unit-form">
        @csrf

        @include('adsense::ad-units._form')

        <div class="form-actions">
            <a href="{{ route('admin.ad-units.index') }}" class="btn btn-secondary">Cancelar</a>
            <button type="submit" class="btn btn-primary">Criar Ad Unit</button>
        </div>
    </form>
</div>
@endsection
